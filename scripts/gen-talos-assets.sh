...
sanitize_k8s_name() {
  ...
  sanitized="$(printf '%s' "${raw_name,,}" | sed -E 's/[^a-z0-9.-]+/-/g; s/^[.-]+//; s/[.-]+$//; s/-+/-/g')"
  if [[ -z "${sanitized}" ]]; then
    sanitized="trusted-root-ca"
  fi
...
if [[ -r "${cluster_k8s_net_constants_path}" ]]; then
  tls_source="$(awk -F'"' '/^[[:space:]]*tls_source[[:space:]]*=/{print $2; exit}' "${cluster_k8s_net_constants_path}")"
  tls_source="$(trim "${tls_source}")"
  root_ca_crt_raw="$(awk -F'"' '/^[[:space:]]*root_ca_crt[[:space:]]*=/{print $2; exit}' "${cluster_k8s_net_constants_path}")"
  root_ca_crt_raw="$(trim "${root_ca_crt_raw}")"
fi

rm -f "${trusted_roots_patch_path}"
if [[ "${tls_source}" == "preissued" && -z "${root_ca_crt_raw}" ]]; then
  echo "Error: tls_source=preissued requires root_ca_crt in k8s_net_constants.tf so Talos can trust the environment CA." >&2
  echo "Fix: set root_ca_crt to a readable PEM certificate path before generating Talos assets." >&2
  exit 1
fi
if [[ -n "${root_ca_crt_raw}" ]]; then
  root_ca_crt_path="$(resolve_cluster_path "${root_ca_crt_raw}")"
  if [[ ! -r "${root_ca_crt_path}" ]]; then
    echo "Error: root_ca_crt is not readable: ${root_ca_crt_path}" >&2
    echo "Fix: point root_ca_crt in k8s_net_constants.tf to a readable PEM certificate before generating Talos assets." >&2
    exit 1
  fi

  root_ca_certificates_block=""
  while IFS= read -r line || [[ -n "${line}" ]]; do
    root_ca_certificates_block+=$'\n    '"${line}"
  done < "${root_ca_crt_path}"

  if [[ -z "$(trim "${root_ca_certificates_block}")" ]]; then
    echo "Error: root_ca_crt is empty: ${root_ca_crt_path}" >&2
    echo "Fix: provide a PEM-encoded certificate with content in root_ca_crt." >&2
    exit 1
  fi

  root_ca_patch_name="$(sanitize_k8s_name "trusted-$(basename "${root_ca_crt_path%.*}")")"
  {
    printf 'apiVersion: v1alpha1\n'
    printf 'kind: TrustedRootsConfig\n'
    printf 'name: %s\n' "${root_ca_patch_name}"
    printf 'certificates: |%s\n' "${root_ca_certificates_block}"
  } > "${trusted_roots_patch_path}"
  echo "wrote ${trusted_roots_patch_path}"
elif [[ "${tls_source}" == "ca_issuer" ]]; then
  echo "warning: root_ca_crt is not configured, so TrustedRootsConfig was skipped and Talos will not trust an autogenerated CA at first boot." >&2
fi
