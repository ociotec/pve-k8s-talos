...
- When the user asks to update a real cluster directory to match current repository changes or newly required constants, follow `docs/agent-workflows/update-cluster-from-repo.md`.
  ...
  cluster; do not assume the section set is unchanged, and ask before creating,
  removing, or deploying newly detected sections.
...
- When the user asks to refresh monitoring sizing values, build a cross-cluster sizing table, or compare current requests/limits/PVCs against real usage, follow `docs/agent-workflows/cluster-monitoring-sizing-table.md`.
...
- When the user asks to audit workload compliance with the Kyverno resource, probe, and exception policy design, follow `docs/agent-workflows/cluster-workload-admission-compliance.md`.
